...
| Policy matcher | 已消歧；ContentOrigin 多值同一-origin 语义已补充；TaskScope resource containment §6.9.1 已闭合 | PolicyRule/PermissionDecision/ApprovalRecord Schema | `domain-policy` 纯领域实现；公开单项 `normalize_uri` / `normalize_uri_pattern`；公开 `resource_refs_within_task_scope` 纯 containment；`kernel-sqlite` 提供 transaction-bound 生产 RateLimitPort | URI/glob、公开 normalizer Task fixture、排序、Condition v1；TaskScope include/exclude/invalid 全量验证；SQLite 原子消费、多连接争最后 slot、winner-only integration | Default Allow；Stop/Recovery 独立 Blocked；containment 不授权；无 PermissionDecision repository |
