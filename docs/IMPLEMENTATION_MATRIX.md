...
| KCP 首批八方法 | 已定义；task.create 四个 repository 阻塞契约已拍板 | 8 组 request/response Schema + task.create hash fixture | `kernel-sqlite` 已实现 task.create/get repository；无 server/handler | normalization/hash、幂等、关系/FK、rollback/tamper | Delegation 非 null 正向路径未实现；Task list cursor 编码未拍板 |
