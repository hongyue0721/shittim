...
| Child Task authority | ADR-0006 accepted；fixture/task-creation owner合同已闭合 | ChildTaskProposal/NormalizedChild/Allocation v1 Schema与generated types已落地；`expires_at`双门Schema已实现，Delta/Authorization/Observation等其余仍未开始 | `kernel-task-creation`已实现child proposal normalization/hash与allocation validation；direct-child v1 repository仍是legacy write path，active materializer未实现 | Schema conformance、inline child normalization/hash/allocation domain tests已覆盖；official child/allocation fixture尚未实现 | 本阶段crate只含proposal/receipt与allocation；authorization projections另切片 |
...
| Schema生成链 | ADR accepted；manifest v2/walker/transaction完成；component-native exact、五值compatibility、MethodVersionBinding完整validator、V2 Envelope authority、production stage gate与task creation fixture合同已闭合 | production=53 entries（41 retained + 12 component-native）；`expires_at` pattern+format双门已生成；production bindings `[]`；三份fixture wrapper明确不是business Schema | schema-tool生成Rust四生成物；中立strict RFC6901 selection/mutation、selected validate/canonicalize library与CLI `--pointer`、Bytes/Hex/Hash模式已实现；`kernel-task-creation` pure library已实现；三份official fixture与Task-specific harness未实现 | pointer unit、resolve复用回归、CLI stdout/mutex/error分层/RFC8785测试及business-v2/graph、timestamp、normalization/hash/allocation inline tests通过；official fixture tamper harness未开始 | 只标通用CLI/library完成，不标fixture/repo/handler/cutover完成；production bindings empty是阶段事实 |
