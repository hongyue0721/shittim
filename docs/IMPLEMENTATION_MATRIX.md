...
| Schema生成链 | ADR accepted；manifest v2/walker/transaction完成；component-native exact、五值compatibility、MethodVersionBinding完整validator、V2 Envelope authority、production stage gate与task creation fixture/CLI合同已闭合 | production=53 entries（41 retained + 12 component-native）；`expires_at` pattern+format双门已生成；production bindings `[]`；三份fixture wrapper明确不是business Schema | schema-tool当前生成Rust四生成物；`--pointer/--hex`与三份fixture未实现；`kernel-task-creation` pure library已实现 | business-v2/graph、timestamp、normalization/hash/allocation domain inline tests通过；official fixture tamper harness未开始 | 不标fixture/repo/handler/cutover完成；production bindings empty是阶段事实 |
