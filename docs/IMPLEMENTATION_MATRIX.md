...
| Schema生成链 | ADR accepted；TaggedUnion IR/Rust renderer 已落地，v2 breaking与MethodVersionBinding设计已裁决 | 当前41个source/manifest均为v1对象；仅task.create request v1进入legacy lifecycle，其余方法/对象v1仍各按active合同 | schema-tool/kernel-contracts生成v1类型；受限 object `oneOf` 可 lower 至中立 TaggedUnion 并生成严格 internally tagged Rust enum；3个 source `additionalProperties:true` Envelope payload 的生成 struct 移除错误 `deny_unknown_fields`，保留开放 payload 语义，Envelope root 仍关闭 | schema-tool 94 tests（inline/ref canonical+arm source、nested、one-branch、AP/UEP、nullable/non-discriminated/ref-target negative、raw JSON、SCC cargo compile、CLI四产物no-partial、稳定性） | 不得把“v1”整体等同legacy；active/legacy逐method binding；TS renderer尚未实现 |
