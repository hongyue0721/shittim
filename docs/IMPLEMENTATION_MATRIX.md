...
|---|---|---|---|---|---|
| Task/Action 状态机 | 已消歧 | 已有首批类型 Schema；生成 `TaskStatus::ALL` / `ActionStatus::ALL` | `domain-task` 纯领域实现；无手写完整状态目录 | 45 tests：NxN、证据、proptest 均直接消费生成闭集 | plan_version=0；success 字符串多重集合；parent_action_id 是补偿唯一事实 |
...
| Policy matcher | 已消歧；ContentOrigin 多值同一-origin 语义已补充；TaskScope resource containment §6.9.1 已闭合；普通未匹配与真实错误 typed 分离 | PolicyRule/PermissionDecision/ApprovalRecord Schema | `domain-policy` 纯领域实现；内部私有 `MatchOutcome`/`MatchResult`；公开单项 `normalize_uri` / `normalize_uri_pattern`；公开 `resource_refs_within_task_scope` 纯 containment；`kernel-sqlite` 提供 transaction-bound 生产 RateLimitPort | URI/glob、公开 normalizer Task fixture、排序、Condition v1；普通 nonmatch Default Allow；旧 sentinel message 经 RateLimitPort 仍 Error；enabled invalid fail closed / disabled invalid 忽略；TaskScope include/exclude/invalid 全量验证；SQLite 原子消费、多连接争最后 slot、winner-only integration | Default Allow；Stop/Recovery 独立 Blocked；containment 不授权；无 PermissionDecision repository；公开 API 无变化 |
...
| Event/SQLite Outbox | 已消歧；sequence 首条已提交为 0、后续连续 +1、单次 append 失败自回滚且不占号；`mark_delivered` 统一 `BEGIN IMMEDIATE` commit 语义 | EventEnvelope + 3 payload；task.created 示例 sequence=0 | `kernel-sqlite` 文件 migration、内部 savepoint 原子 sequence/position、cursor、at-least-once delivery storage；`mark_delivered` convenience 委托统一写事务；task.create 同事务生产唯一 task.created | 多聚合/多连接竞争/忽略 append 错误仍不留空洞/payload mismatch/cursor/重启前后 at-least-once；mark Err/panic rollback 重试、unhealthy fail closed、busy 争用、双 store 争 position 恰好一 Marked；task.create 完整 Envelope/payload 断言；`kernel-sqlite` 44 tests | 无 Publisher 循环、retention 或 claim lease；Task list/update 未实现 |
...
| Schema 生成链 | ADR accepted | 41 个 source + manifest；task.create executable fixture 不新增 Schema | schema-tool + kernel-contracts；`id_base` 权威 namespace；URI/`Url::join`+percent-decode→RFC6901；`RustProjection` 单次 project（公开仅 `project_rust` + `render_*_from_projection` + catalog）；use-site lineage + SCC `Option<Box<T>>`；`GeneratedArtifact`/`ArtifactPlan` 字段 private，`try_new` 唯一 plan 构造；envelope 0-ref untyped / ≥1 双射；optional/non-null serde omission；string enum `ALL` 闭集 + 自动合同测试；`domain-task` 已删除手写 status catalog 并直接消费生成闭集 | meta/$ref/drift/JCS + task.create hash + optional omission + targets/closure/collision/artifact `try_new` + id_base namespace（default-port/dot-segment/double-slash/percent path）+ sibling/diamond projection + mixed envelope mapping + relative external missing target + local/abs/rel alias + percent/anchor/nested $id + recursive offline cargo check（schema-tool lib 29 + graph_projection 24 + cli_smoke 36 = 89；kernel-contracts lib 含全部 string_enum_contracts + contract_validation 代表性 ALL API；domain-task 45 tests 直接遍历 ALL） | 当前只输出 Rust 四文件；TS renderer 仍未实现；Approval target exactly-one 仍未由 Schema 强制 |
