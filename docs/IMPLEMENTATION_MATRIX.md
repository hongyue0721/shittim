...
| Schema生成链 | ADR accepted；TaggedUnion IR/Rust renderer已落地；**manifest v2 root/component/retained-ID namespace migration已完成**，含production immutable 41-ID source ledger、authoritative SchemaNode identity index、统一walker与单root transaction；**artifact transaction 的 typed trace、LockPort exact 矩阵与 TransactionFs fixed-point exact 矩阵已完成独立验收**；业务v2 breaking与未来MethodVersionBinding结构已裁决 | manifest仅接受v2 root `https://schemas.shittim.local/`；6个显式components的namespace/`allowed_refs`/retained ownership；required typed `method_version_bindings: []`是唯一当前允许值，41个source与`/v1/` `$id`由生产load source SHA-256 gate持续证明未变；四个Rust生成物未变仅为`7fb25cf`验收记录 | schema-tool以单一SchemaNode walker覆盖restricted Schema-bearing map/single/array并为每document保存private immutable pointer index；ArtifactPlan只接受恰好一个root；当前 transaction 为单root/Linux real-platform verified，使用持久OS advisory lock、`Preparing/Prepared/RollingBack/Committed` durable journal及同filesystem stage/backup/discard；structured `FailureDisposition` 已包含 `StoredStateInvalid`，正式journal损坏时fail closed且不改变root | walker全location表、instance-data/reserved-name与ref→const/default/examples/enum负例、合法defs/properties/items、local/relative/absolute/cycle、dynamicRef三CLI load gate；LockPort exact operation/failure/ownership矩阵；TransactionFs typed trace、即时完整snapshot、reference model、CrashBefore/CrashAfterSuccess/IoNoEffect/声明的IoPartial、recover fixed-point与腐坏journal fail-closed | **未**实现业务v2 Schema、可用binding生成/八方法表、TS renderer、KCP切换、multi-root transaction或non-Linux real-platform port；control-flow/fault conformance不等同真实断电介质模型；同用户恶意FS mutation不在transaction conformance安全边界；不得把manifest v2误写为业务v2 |
