...
| Child Task authority | ADR-0006 accepted | ChildTaskProposal/NormalizedChild/Allocation v1 Schema与generated types已落地；Delta/Provenance等其余仍未开始 | direct-child v1 repository是legacy write path；active入口未实现 | Schema conformance 已覆盖 proposal/allocation shape | TaskSpec v1仍可读active/legacy Task；新child唯一Action入口 |
...
| Schema生成链 | ADR accepted；manifest v2/walker/transaction完成；component-native exact、五值compatibility、MethodVersionBinding完整validator、V2 Envelope authority、`validate_production_manifest_stage`、authority `KCP_ENVELOPE_AUTHORITY_METHODS`/legacy `KCP_LEGACY_V1_*`/binding catalog **library + first 12 business schemas implemented** | production=53 entries（41 retained + 12 component-native）；production bindings `[]`；retained 4项compatibility演化标签已改标，binding lifecycle与之正交；Alias resolution/audit、format assertion/unknown fail-closed、validated decode、open-object无损round-trip及member collision audit、integer neutral bounds与u32准确类型已落地 | schema-tool当前生成Rust四生成物；types/typed含12 root；无TypedKcp*EnvelopeV2 | business-v2 conformance、real mutant、Alias root/chain/pure-cycle/nullable/recursive、shared format与unknown-format、u32 max/overflow/type、decode taxonomy、collision fail-closed、synthetic 8-method、production empty stage覆盖 | 不标official fixtures/hash/repo/handler/cutover完成；production bindings empty是阶段事实 |
