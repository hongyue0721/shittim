...
| KCP Envelope / Value preflight | active合同使用Command/Query Envelope V2结构门 + MethodVersionBinding业务门；合同已闭合 | 首批12项尚未创建；production bindings空；当前v1 envelopes retained未改 | 当前Rust preflight错误地全局只接受v1，属于待替换实现 | 当前测试只覆盖旧全局v1行为 | synthetic 8-method validator/catalog也未实现；active task.create只接受2；升级前禁止server |
...
| Schema生成链 | ADR accepted；既有manifest v2/walker/transaction已完成；**首批12 Schema、component-native exact目标（一般title-derived；KCP envelope固定去`Kcp`前缀得到`command_envelope`/`query_envelope`）、五值compatibility、canonical `$defs`宿主、Envelope registry发现、active Catalog命名与MethodVersionBinding stage gate合同已闭合** | 当前仍只有41 retained source/entries；production bindings `[]`；12新Schema与compatibility改动未应用 | 当前loader仍empty-only且component-native仅namespace级；Catalog仍有suffix发现/`KCP_V1_METHODS`代码事实；通用非空validator、exact硬门、`validate_production_manifest_stage`均未实现 | 后续必须用synthetic 8-method registry测试完整closure；非空synthetic end-to-end plan/render走library API与显式non-production registry profile，production CLI始终经production-empty gate | 本轮只改docs/FILE_MANIFEST；不改manifest/source/Rust/generated；这里只标合同闭合，绝不标代码实现 |
