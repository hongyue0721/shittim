...
| Child Task authority | ADR-0006 accepted；fixture/task-creation owner前置合同已闭合 | ChildTaskProposal/NormalizedChild/Allocation v1 Schema与generated types已落地；`expires_at`双门Schema及Delta/Authorization/Observation等其余仍未开始 | direct-child v1 repository是legacy write path；active入口、`kernel-task-creation`均未实现 | Schema conformance已覆盖现有proposal/allocation shape；新时间门与official child/allocation fixture尚未实现 | 本阶段crate只含proposal/receipt与allocation；authorization projections另切片 |
...
| Schema生成链 | ADR accepted；manifest v2/walker/transaction完成；component-native exact、五值compatibility、MethodVersionBinding完整validator、V2 Envelope authority、production stage gate与task creation fixture/CLI前置合同已闭合 | production=53 entries（41 retained + 12 component-native）；production bindings `[]`；三份fixture wrapper明确不是business Schema | schema-tool当前生成Rust四生成物；`--pointer/--hex`、三份fixture与`kernel-task-creation`均未实现 | 现有business-v2/graph测试通过；fixture tamper/allocation domain测试未开始 | 不标fixture/hash/repo/handler/cutover完成；production bindings empty是阶段事实 |
| Rust workspace | ADR accepted；已拍板未来增加`kernel-task-creation` | 不适用 | 当前仍为kernel-contracts、schema-tool、domain-task、domain-policy、kernel-sqlite、kernel-kcp；`kernel-task-creation`未创建 | fmt/clippy/workspace test | 冻结范围：task proposal/receipt/idempotency/allocation；URI复用domain-policy唯一实现，全部事实caller注入；authorization projections另切片 |
