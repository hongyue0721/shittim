...
| KCP 首批八方法 | 已定义；task.create 四个 repository 阻塞契约已拍板 | 8 组 request/response Schema + task.create hash fixture | URI 单项 normalizer 已可复用；仍无 server/handler/repository | 方法闭集、payload 绑定、task.create JCS/projection fixture；domain-policy 重放 normalization fixture | Delegation 非 null 正向路径未实现；Task list cursor 编码未拍板 |
