...
| KCP Envelope / Value preflight | active合同使用Command/Query Envelope V2结构门 + MethodVersionBinding业务门；合同已闭合 | 首批12项尚未创建；production bindings空；当前v1 envelopes retained未改 | schema-tool library已实现claimant式V2 authority、target-local binding facts、active/legacy正交catalog与typed version selector；kernel-kcp仍走retained v1 preflight | synthetic authority/binding/跨target/lifecycle负例已覆盖；runtime cutover测试未开始 | active task.create只接受2；升级前禁止server |
...
| Schema生成链 | ADR accepted；manifest v2/walker/transaction完成；component-native exact、五值compatibility、MethodVersionBinding完整validator、V2 Envelope authority、`validate_production_manifest_stage`、active `KCP_METHODS`/legacy `KCP_LEGACY_V1_*`/binding catalog **library implemented** | 当前仍只有41 retained source/entries；production bindings `[]`；12新业务Schema尚未落地；retained 4项lifecycle已改标 | schema-tool通用loader接受合法非空binding；production CLI load后plan前stage gate断言empty；catalog不再用suffix/`KCP_V1_METHODS`作active总目录 | synthetic 8-method load/lower/render与负例、component-native正反例、production empty+lifecycle、types/typed/mod byte稳定均已覆盖 | 不标12 Schema/repo/handler/cutover完成；production bindings empty是阶段事实 |
