...
> 状态：当前41个source与Rust生成物的对象版本均为v1，但不能整体标legacy：`task.create` request v1是legacy validation-only，其余首批method v1仍active；共享Actor/状态等v1继续被active合同引用；Approval/PD/EventEnvelope等在v2落地后按各自lifecycle转legacy。ADR-0006/0007要求的v2尚未生成。
...
| Manifest | `schemas/manifest.json` | `$id`、source、kind、兼容与 `generation_targets`（当前 41 entries 均为 `[rust]`）；当前没有 Computer Use Profile 生成 entry 或生成包；**`id_base`** 是权威 URL path 命名空间：必须 canonical absolute `http(s)`、无 fragment、以 `/` 结尾；每个 entry `$id` 必须落在该 namespace（scheme/host/port/path 组件语义，禁止字符串前缀伪装）；root `$id` / manifest id 必须 canonical absolute `http(s)`、无 fragment |
...
- active v2 source/manifest entries与generated `MethodVersionBinding` catalog（`task.create active=[2], legacy=[1]`；其余首批方法active=[1]）；
- `SchemaGraph` source profile精确定义：object `oneOf` branch discriminator required const双射、封闭unknown-field策略、nested union/`$ref` resolution、ordinary nullable oneOf与Envelope conditional分析隔离、unsupported fail closed；lower为`TypeShape::TaggedUnion`，Rust serde enum/未来TS union共用IR；
- TaskCreateRequest/Response v2 root-only；
- manifest namespace按IC §13.6从`https://schemas.shittim.local/v1/`迁移到component namespace base，保留现有`$id`并通过retained-id/component/ref gate；
