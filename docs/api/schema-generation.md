...
> 状态日期：2026-07-18。Manifest v2/walker与transaction基座已完成；首批12 Schema/compatibility/component-native ID/MethodVersionBinding两阶段合同现已在docs闭合，但Schema source、manifest entries、通用非空binding validator/catalog和业务代码均未实现。production `schemas/manifest.json` bindings仍为空；当前empty-only loader是待替换代码事实，不是目标通则。41个retained source/ID/bytes保持不变。
...
| Manifest | `schemas/manifest.json` | 当前production事实仍为41 retained entries与显式空`method_version_bindings`；现有loader仍empty-only，component-native仍只有namespace membership等较宽校验，Catalog仍有suffix/`KCP_V1_METHODS`代码事实。后续Schema/工具阶段必须改为通用完整非空validator、`SchemaRegistry::load` exact component-native硬门、registry V2 Envelope唯一发现、active `KCP_METHODS`目录与独立`validate_production_manifest_stage`。compatibility目标闭集为`v1-stable/new-contract/breaking-replacement/legacy-validation-only/legacy-read-only`，与binding lifecycle正交；synthetic/test manifest也无额外值。新component-native ID精确为`https://schemas.shittim.local/<component>/<snake_case_name>/vN`，source镜像`schemas/source/<component>/<snake_case_name>.vN.json`，新ID不进41项ledger；精确12项矩阵见IC §13.6。当前manifest/source/Rust尚未应用这些后续改动。 |
...
- KcpCommandEnvelopeV2/KcpQueryEnvelopeV2保持protocol 1.0，0个method payload conditional `$ref`；只禁止这两个generic typed decode及其0-ref同义wrapper，不笼统禁止其它`Typed*V2`；active family authority由registry exact V2 Envelope facts唯一选择，不按ID suffix；active常量为`KCP_COMMAND_METHODS`/`KCP_QUERY_METHODS`/`KCP_METHODS`；binding选择业务payload；
...
- 上述12个Schema source/entries、compatibility迁移与生成物；
- 通用合法非空MethodVersionBinding validator、synthetic 8-method tests、registry exact active Envelope发现、`KCP_METHODS` active catalog与generated binding catalog；当前loader仍empty-only且catalog仍有suffix/`KCP_V1_METHODS`代码事实；
- `validate_production_manifest_stage`及check/generate挂载；
- current `SchemaRegistry::load`仍empty-only且component-native仅namespace级，generated Catalog仍保留suffix/`KCP_V1_METHODS` active命名代码事实；这些必须由后续工具提交替换，不能因为合同闭合而标完成；
