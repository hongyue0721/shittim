...
> 状态日期：2026-07-18。Manifest v2/walker/transaction基座已完成；**schema-tool library**已实现正式五值compatibility与kind组合门、精确production lifecycle ledger、component-native/source-title/root schema_version硬门、claimant式V2 Envelope authority、registry-level binding验证、TargetPlan-owned target-local authority/binding facts、active/legacy正交catalog、typed request-version selector与显式Production/Synthetic registry profile。production `schemas/manifest.json` bindings仍为空；12业务Schema source/entries、kernel-kcp runtime cutover与可连接server仍未实现。41个retained source/ID/bytes保持不变。
...
| Manifest | `schemas/manifest.json` | production仍为41 retained entries与显式空`method_version_bindings`。**library implemented**：正式五值compatibility；component-native exact ID/source/title/version硬门；完整非空MethodVersionBinding validator；registry V2 Envelope唯一发现；production CLI `check`/`generate`在load后plan/render前调用`validate_production_manifest_stage`。active目录为`KCP_COMMAND_METHODS`/`KCP_QUERY_METHODS`/`KCP_METHODS`；retained v1目录显式为`KCP_LEGACY_V1_*`；binding catalog为`METHOD_VERSION_BINDINGS`。12业务Schema尚未落地。 |
...
- 正式五值`compatibility`（无test-only旁路）；retained 4项lifecycle改标，其余37 `v1-stable`；
- component-native exact硬门与`schema_version_field`完整const校验；
- 通用合法非空MethodVersionBinding validator + synthetic 8-method tests；
- registry exact active Envelope authority（删除ID suffix发现）；
- `validate_production_manifest_stage`挂production CLI；
- catalog：`KCP_METHODS` active与`KCP_LEGACY_V1_*` retained正交共存，`METHOD_VERSION_BINDINGS`只来自当前target完整facts；生成`RequestVersionSelection::{Active,LegacyValidationOnly,Unsupported}`，Active同时绑定request/response，不让caller重新拼数组。

## 明确未实现

- 上述12个Schema source/entries与对应生成物；
