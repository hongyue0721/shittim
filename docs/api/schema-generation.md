...
| JCS 向量 | `schemas/examples/jcs/`、`schemas/fixtures/kcp/task_create_normalized_hash.v1.json` | 现有 RFC 8785 示例与既有legacy fixture；新合同已固定三份未来路径：root `schemas/fixtures/kcp/task_create_normalized_hash.v2.json`、child `schemas/fixtures/task/child_task_proposal_normalized_hash.v1.json`、allocation `schemas/fixtures/task/task_creation_allocations.v1.json`，当前文件仍未创建 |
...
fixture切片要求但当前CLI尚未实现的中立接口：`validate`与`canonicalize`都接受strict RFC6901 `--pointer <pointer>`，先从输入JSON选择目标值；pointer空串表示document root，非法`~`escape、非法array index、缺失member/index都结构化失败。`canonicalize`支持`--hex`输出JCS UTF-8 lowercase hex，`--hash`输出SHA-256 lowercase；`--hex`与`--hash`互斥。`schema-tool`不得识别Task字段、执行URI/time normalization、构造projection或解释tamper业务语义；这些属于未来`kernel-task-creation`与fixture harness。
...
## Task creation fixture与生产owner合同（尚未实现）

三份official测试制品路径、外层字段名与tamper结构以IC §5.3.1/§6.10.6为唯一事实源。JCS对象只保存`jcs_utf8_hex`与`sha256`，不重复canonical string；allocation fixture不保存JCS/hash。wrapper是测试制品合同，不是Draft 2020-12 business Schema，不进入manifest、不生成类型。

正式生产task creation owner命名固定为新纯crate `kernel-task-creation`。本阶段范围只含root/child proposal normalize、root receipt/idempotency projection与hash、child proposal/receipt hash、root/child allocation validation；`ChildTaskDeltaProjectionV1`、`MaterialAuthorizationProjectionV1`、`ObservationEvidenceProjectionV1`未来由专门authorization projection owner或独立切片实现。crate依赖`kernel-contracts`，并调用`domain-policy`当前唯一权威URI parser/normalizer；不得复制第二实现，长期抽取中立URI crate须另立ADR。全部业务事实由caller typed input注入，crate不依赖SQLite/KCP、不分配ID、不读取repository、不写存储。allocation external refs使用闭合版本化Rust snapshot类型，不是business Schema、不进manifest；字段新增必须编译期更新caller。当前crate/fixture/helper均不存在，不能据本文标成library implemented。

## 首批12 Schema权威导航（合同已闭合，source/generated 已实现；bindings/fixtures/cutover 未完成）
...
## 本切片已实现（library + production empty）
...
- `InputTaskScopeV1.expires_at` source Schema的Draft 2020-12 `pattern + format:date-time`双门与重新生成（当前仅合同闭合；pattern固定大写T/Z、秒`00..59`、Z/offset及零fraction，format负责真实日期/时间/offset合法性）；
- official task creation fixtures（三份固定路径；本阶段仅合同/Schema/typed conformance + inline JSON）；
- `schema-tool validate/canonicalize --pointer`、`canonicalize --hex`及`--hex/--hash`互斥；
- `kernel-task-creation`生产helper crate；
- TaskCreateRequest/Response v2 root-only runtime handler/repository；
