...
`schema-tool`不识别Task字段、不执行URI/time normalization、不构造projection或解释tamper业务语义；这些属于已实现的`kernel-task-creation` pure library与下一切片的official fixture harness。
...
## Task creation fixture与生产owner合同（helper与通用pointer CLI已实现，official fixture尚未实现）
