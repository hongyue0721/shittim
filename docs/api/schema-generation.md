...
> 状态：已落地首批 Rust 生成链。JSON Schema 是唯一人工源；当前没有 TypeScript/Python 生成物。根目录已有零依赖 Node 24.18.0 / pnpm 11.3.0 工作区基座，但**未**接入 Schema→TS 生成。
...
| Typed decode | `generated/typed.rs` | 从 envelope discriminator enum 与 `allOf if/then payload.$ref` 一一映射自动派生；`decode` 先 Schema validation，再调用有明确前置条件的 `decode_after_validation`；wire/payload/discriminator 失败为结构化 stage |
...
- Serde omission 由 Schema 元数据确定性推导，不手改 generated：
  ...
  - `$ref` 只允许 `title` / `description` 注释 sibling；带 `type`、约束或其它 shape sibling 时生成失败，避免忽略 JSON Schema 2020-12 的共同生效语义。
...
- generated `deserialize_wire`、`deserialize_payload` 与 discriminator default 分别产生 `WireDecodeAfterSchema`、`PayloadDecodeAfterSchema`、`GeneratedDiscriminatorMapping`；Catalog/unknown Schema 使用 `SchemaCatalog` 分类。上层不匹配 serde/Schema 文本。
- `decode_after_validation` 是公开低层 API，只允许调用方先用匹配的 generated envelope Schema 验证同一个 `Value`；普通调用继续使用完整 `decode`。
...
- optional/non-null 与 required-nullable 的 wire 语义不同：前者 `None` 省略，后者 `None` 必须是显式 `null`。PolicyRule `confirmation_mode`（allow/deny）、PermissionDecision `approval_type`、Approval target 成员等因此在序列化后可通过 Schema；人工写入 `null` 到 non-null optional 仍会失败。
- ApprovalRecord `target` 当前仍是全部可选成员的对象：空 `{}` 与多成员同时出现在 Schema 层都合法；exactly-one 目标选择是已知缺口，生成器只保证 `None` 成员省略，不伪装已解决 exclusivity。
...
- TypeScript/Python 类型生成（根 pnpm workspace 基座存在，但不生成、不发布 TS 类型）；
- `agentd`、KCP bytes/frame/transport 与任何可运行服务端；Value preflight/registration/dispatcher 已作为不可连接 `kernel-kcp` 库边界实现；
...
Task/TaskScope/ContentOrigin create/get repository 与 `task.create` Policy URI 规范化持久化 producer 已由 `kernel-sqlite` 实现；详见 [`kernel-sqlite.md`](kernel-sqlite.md) 与 [`task-repository-contract.md`](task-repository-contract.md)。
