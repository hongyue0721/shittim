...
fixture切片要求但当前CLI尚未实现的中立接口：`validate`与`canonicalize`都接受strict RFC6901 `--pointer <pointer>`，先从输入JSON选择目标值；pointer空串表示document root，非法`~`escape、非法array index、缺失member/index都结构化失败。`canonicalize`支持`--hex`输出JCS UTF-8 lowercase hex，`--hash`输出SHA-256 lowercase；`--hex`与`--hash`互斥。`schema-tool`不得识别Task字段、执行URI/time normalization、构造projection或解释tamper业务语义；这些属于已实现的`kernel-task-creation` pure library与下一切片的fixture harness。
...
## Task creation fixture与生产owner合同（helper已实现，official fixture/CLI尚未实现）
