...
> 状态：首批12项相关Schema source、manifest entries与generated Rust root types已落地；production MethodVersionBindings仍为空。active v2仍contract-only；三份official fixture路径/wrapper与`kernel-task-creation`职责已经合同闭合，但`expires_at`非零亚秒Schema硬门、fixture、crate、repository canonical write/readback、handler与cutover均未实现。现有Rust/SQLite只实现legacy TaskCreate v1 create/get，不得进入未来production server。
...
生产纯逻辑owner固定为未来crate `kernel-task-creation`。本阶段只拥有root/child proposal normalize、root receipt/idempotency projection与hash、child proposal/receipt hash、root/child allocation validation；`ChildTaskDeltaProjectionV1`、`MaterialAuthorizationProjectionV1`、`ObservationEvidenceProjectionV1`未来由专门authorization projection owner或独立切片实现。crate依赖`kernel-contracts`并调用`domain-policy`当前唯一权威URI parser/normalizer，不复制URI语义；长期抽取中立URI crate必须另立ADR。全部事实由caller typed input注入，crate不读repository、不依赖SQLite/KCP、不分配ID、不写存储。未来`kernel-sqlite`调用它；`domain-task`保持纯状态机且不增加policy依赖。当前crate尚未创建。
...
精确外层字段与tamper结构见IC §5.3.1/§6.10.6。JCS只存lowercase `jcs_utf8_hex`与`sha256`，不存canonical string；allocation fixture不存JCS/hash。wrapper不得升级为business Schema或manifest entry。root tamper固定覆盖auth非null先Schema拒绝、deadline/request/idempotency两hash不变、context只改idempotency、payload改两hash、parent拒绝、URI lexical equivalent不改hash、数组顺序/重复改hash。allocation fixture必须明确包含Schema-valid但domain拒绝的内部UUID重复、external ref碰撞与opaque重复向量；不证明generator非派生性。
