...
已有首批 JSON Schema 源、manifest、Rust 生成类型与校验/哈希 API，以及纯领域 `domain-task` 状态机；**没有**可连接的 `agentd`、没有稳定网络 endpoint、没有 TypeScript 客户端包。
...
`domain-task` 只产出领域转换结果与事件**意图**，不分配真实 `EventEnvelope` 位置，也不构成外部 SDK 表面。
