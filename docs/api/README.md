...
`domain-task` 只产出领域转换结果与事件**意图**；`domain-policy` 只产出非持久 decision draft / canonical input，并显式区分 Stop Fence/Recovery invariant。`kernel-sqlite` 已拥有文件 migration、AuditRecord JSON、Event Outbox 与事务绑定 rate-limit 消费，但尚无 Task/Action/PermissionDecision repository。以上均不构成外部 SDK 表面。
