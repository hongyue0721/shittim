...
已有首批 JSON Schema 源、manifest、Rust 生成类型与校验/哈希 API、纯领域 `domain-task` 状态机、Freedom-first `domain-policy` matcher，以及文件型 `kernel-sqlite` 持久化基座和 Task create/get repository；三个 KCP typed handler 的合同已闭合但代码尚未实现。当前**没有**可连接的 `agentd`、没有稳定网络 endpoint、没有 TypeScript 客户端包。
...
- [Task repository 创建与读取契约](task-repository-contract.md)（create/get 已实现；三方法 typed handler 合同已闭合，代码/list/server 未实现）
...
`domain-task` 只产出领域转换结果与事件**意图**；`domain-policy` 只产出非持久 decision draft / canonical input，并显式区分 Stop Fence/Recovery invariant。`kernel-sqlite` 已拥有文件 migration、AuditRecord JSON、Event Outbox、事务绑定 rate-limit 消费和 Task create/get repository。三个 typed handler 的合同已闭合但 Rust 代码尚无；raw preflight、可连接 server、Task list/update、Action 与 PermissionDecision repository均未实现。
