...
- 状态：accepted
- 日期：2026-07-16
- 修订：2026-07-18（闭合首批12 Schema合同；同日后续工具切片实现compatibility/component-native/MethodVersionBinding library + production empty stage gate；12 Schema source仍未落地）
...
当前代码事实：`SchemaRegistry::load`已接受并完整验证合法非空binding；component-native exact硬门与正式五值compatibility已落地；active catalog由V2 Envelope authority发现并生成`KCP_METHODS`，retained v1为`KCP_LEGACY_V1_*`；production empty由`validate_production_manifest_stage`在CLI check/generate挂载。12业务Schema source与production非空binding仍未启用。
