...
- 状态：accepted
- 日期：2026-07-16
- 修订：2026-07-18（闭合首批12 Schema、component-native ID、compatibility与MethodVersionBinding两阶段合同；代码未实现）

## 背景

当前规范要求区分两代：

- active TaskCreateRequestV2 root-only、Approval/PermissionDecision及Causation/Event/Audit相关v2：尚无source Schema/生成物；
- 当前41个source/生成类型的对象版本都是v1，但lifecycle必须逐Schema/逐method判断：retained `TaskCreateRequestV1`后续标`legacy-validation-only`；其余首批KCP request v1仍active；Actor/TaskStatus/TaskSpec等共享v1按引用方合同继续active；retained `KcpCommandEnvelopeV1`/`KcpQueryEnvelopeV1`后续标`legacy-validation-only`，`TaskCreateResponseV1`标`legacy-read-only`。禁止写成“所有v1都legacy”。
...
当前代码事实仍是：`SchemaRegistry::load`拒绝任意非空binding，component-native entry只验证namespace membership等较宽条件，Catalog按Envelope ID suffix发现且active总目录仍叫`KCP_V1_METHODS`。本ADR只规定后续替换目标，不把这些代码项标为已完成。
...
- 当前41个manifest entry仍只声明`[rust]`并只输出Rust四文件；manifest已升级为v2，使用root id_base、component namespace、`allowed_refs`、retained-ID ownership及不可变迁移ledger；41份source `$id`由fixture source hash持续证明相对迁移前基线字节不变。四份Rust生成物的迁移不变性只记录为`7fb25cf`验收证据，持续drift由正常generate/check与Git基线审阅。production `method_version_bindings: []`仍是当前阶段事实，且本次仅闭合docs合同；通用非空validator、12个Schema、manifest compatibility改动、binding catalog与任何业务实现都尚未落地。对象版本均为v1的当前代码不能按版本号整体标legacy。TypeScript目标模型与闭包校验已具备，但TS代码生成尚未实现，不得手写平行类型。
