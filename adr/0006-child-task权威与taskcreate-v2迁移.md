...
现有 `TaskCreateRequest v1` 同时允许 `parent_task_id = null` 与非 null，因此同一个公开 KCP Command 既能创建根 Task，也能直接创建子 Task。与此同时，旧规范又写过“Child Task创建本身是一个Action”。这造成两个生产入口、两个因果模型和无法闭合的权限事实：直接 KCP 创建子 Task 时，没有一个父 Task 所拥有的 Action 可以承载 Policy、Lease、Stop Fence、Verification、重放与恢复。
...
- active KCP `task.create` 切换到 `TaskCreateRequest v2`，只创建根 Task。
- v2 payload 不包含 `parent_task_id`；Envelope `task_id` 与 `expected_revision` 必须为 `null`。
- `TaskCreateRequest v1` 永久冻结为 legacy validation/read/migration 合同，不再进入 active dispatcher，也不接受 production write。
...
Action 的 `structured_arguments` 使用版本化 `ChildTaskProposal v1`，完整声明 child 的 proposer、goal、constraints、success criteria、risk/capability hints、完整 TaskScope、显式 Delegation 选择和 ContentOrigin 输入。调用方不得提供 child Task/Scope/Origin/Receipt/Audit/Event/Verification ID，不得提供 `parent_task_id`、status、plan version、revision、created/updated time或 Action completion 字段。
...
- manifest后续生成的`MethodVersionBinding`完整规则（active非空、legacy可空、升序唯一互斥、map key并集与response规则）以IC §13.5为准；本ADR只记录task.create的breaking lifecycle：active request `2`、legacy validation `1`，其余七方法当前active `1`；
...
`TaskCreateRequest v2` 的 receipt hash 与 idempotency projection均以规范化后的 v2 payload为输入；不得把 v1 `parent_task_id` 的 null 占位带入 v2 hash。精确字段在 `IMPLEMENTATION_CONTRACTS.md` 定义。
...
后续实现必须新增 v2 Schema/生成类型、TaskCreate v2 handler、child materialization repository、Action/PermissionDecision/Approval repositories、CausationRef/EventEnvelope/ContentOrigin/Audit v2、migration/provenance 与 Conformance。现有 v1 Rust/Schema/SQLite 事实不因本 ADR 自动改变，当前 production dispatcher 仍未达到本决策。
