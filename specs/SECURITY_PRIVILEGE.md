...
这些是可由规则选择的隐私能力，不能成为对认证界面、敏感画面或云模型的绝对发送禁令。发送到模型或远程 Channel 时，以当前任务相关性最小化内容，并记录最小必要元数据、ContentOrigin 和适用规则。
...
- 目标必须绑定窗口/快照；
- 当前焦点不能作为唯一目标；
- Agent 宿主、审批和认证界面属于 Protected Surface，其观察、输入、远程发送和云调用行为由匹配 Policy 决定；
- 视觉低置信度是可靠性事实和 Policy/verification 输入，不会自行导出 confirm 或 deny。
...
用户或已获规则授权的 actor 必须能触发 Kernel 级 Emergency Stop。Stop Fence 是 Kernel 持久、原子且可审计的拦截状态：启用后，任何新副作用、主动任务、输入、Extension 调用、远程执行和 Privilege Lease 消费都必须在边界处拒绝；正在执行的可取消工作立即取消，不能安全取消的工作进入恢复与验证。停止不依赖模型响应，且外部内容不能清除或伪造 Stop Fence。
