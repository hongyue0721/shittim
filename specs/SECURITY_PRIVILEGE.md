...
- Screenshot、Protected Surface 的观察/输入/远程发送/云调用和视觉低置信度，均是该 Profile 提供的标签、来源与证据引用；Profile 不自行导出 allow、confirm 或 deny。Kernel 必须把当前事实纳入 PermissionDecision evaluation context，事实变化使旧 decision、approval 与相关 Permission/Privilege Lease 失效并回 Core 重评；
