...
可作为可替换 Provider / Adapter，默认不强制安装：

- A_Memorix（Memory Provider 候选）；
- OmniParser（Visual Grounding Provider）；
- computer-use-linux（Linux Computer Use Provider）；
- pi-computer-use（Windows/macOS Provider 或架构参考实现）；
- Wasmtime/WASI（可选低权限扩展运行时）；
- Set-of-Mark 方法（标注策略，通常是方法而非独立服务）。
...
- 跨平台 Computer Use Core；
- Snapshot Composer；
- Execute Gate / Input Lease；
- 权限；
- Task 和验证。

## 6. pi-computer-use

分类：optional provider / architecture reference。
...
候选必须进入统一 Target Resolver，与语义/应用候选融合，并接受 Execute Gate 约束。

## 8. Set-of-Mark

分类：optional method / annotation reference。
...
| 截图点击工具链 | Computer Use + Execute Gate | `COMPUTER_USE.md` |
...
避免把 Computer Use 的 Execute Gate 与 MaiBot 历史 Turn/Action Gate 混称。
