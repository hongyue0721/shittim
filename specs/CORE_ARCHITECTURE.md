...
所有持久化事件包装在 EventEnvelope 中。`sequence` 只表达同一聚合内的领域顺序；`outbox_position` 是事件写入 Outbox 时由 Kernel 分配的全局单调递增投递位置，只用于发布、分页和 cursor，不代表跨聚合因果顺序。
...
- 聚合内事件仍按 `sequence` 严格有序；`outbox_position` 提供全局投递顺序，但不声明跨聚合领域因果关系；
