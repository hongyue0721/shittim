...
raw Envelope/schema preflight 可以返回：
...
本节是这三个方法的库级 application handler 唯一事实源。它只闭合已经通过对应 Command/Query Envelope Schema、方法 payload Schema和 `kernel-contracts` typed decode 的请求；UTF-8、JSON parse、frame、最大尺寸、protocol/auth/method/schema preflight 和未知方法分派不在本批。实现不得在此层接受 raw JSON 或 transport frame，也不得把 typed handler 内部错误伪装成 `invalid_request`。
...
本批合同只允许未来新增一个不可连接的库级 typed handler crate/module及其 fake-port 测试。raw parse/frame/preflight、Catalog 全方法可用性、Event/Stop、`task.list` cursor、transport path/pipe name、连接 ACL/credentials 和 server 生命周期未关闭前，不得提供可连接 Socket/Named Pipe server，不得把仅实现三个方法描述为首批 KCP server 可用，也不得用 `method_unavailable` 掩盖 Catalog 方法缺失。
