...
- 接受时间：typed application handler 的第一次 `KernelClock.now_utc()` 读取同时作为 deadline 入口检查时间和本次 `accepted_at`；下述所有创建时间均使用这个值，不允许各对象分别取时钟。完整读取点见第 5.10 节；
- 规范化：只规范化非 null `origin.source_uri`、`task_scope.resource_patterns[]` 与 `task_scope.exclusions[]`，统一使用 `SECURITY_PRIVILEGE.md` 的 Policy URI 语法。数组顺序与重复项原样保留；其他字符串不 trim、不排序、不去重。URI 或 Pattern 规范化失败返回 `invalid_scope_pattern`；规范化后的完整 payload 必须再次通过 `TaskCreateRequest` Schema，若该内部 post-normalize contract 门失败则按第 5.10.5 节折叠为 `internal_error`，不得重新伪装成 raw `invalid_request`；
...
- 错误：`invalid_request`（仅 raw/schema preflight）、`invalid_scope_pattern`、`delegation_not_found`、`parent_task_not_found`、`parent_origin_not_found`、`idempotency_conflict`、`sqlite_busy`、`sqlite_full`、`sqlite_corrupt`、`stored_data_invalid`、通用错误；typed handler 的稳定映射见第 5.10 节。
...
raw `serde_json::Value` preflight 可以返回下列 wire error；精确分类、固定 message、response eligibility 与本地拒绝见第 5.11 节：

- `invalid_request`：Envelope 或 payload 不满足 Schema，或本节明确归为 caller structural invalid；
- `unsupported_protocol_version`；
- `unsupported_schema_version`；
- `unsupported_method`；
- `unsupported_auth_schema`。

这些错误发生在 typed application handler 之前；已经通过对应 Command/Query Envelope Schema 与 typed decode 的请求不得在 handler 内重新归类为 `invalid_request`。
...
- `sqlite_busy`：请求期内无法取得 SQLite 锁；
- `sqlite_full`：SQLite 或承载文件系统无可用空间；
- `sqlite_corrupt`：SQLite 报告损坏或文件不是有效数据库；
- `stored_data_invalid`：已存 canonical JSON 或关系投影未通过完整性验证；
- `internal_error`：未分类 Kernel、constraint、contract、serialization、migration/configuration 或组合错误，不得泄漏 Secret。

方法专属错误见各条目。`error.message`、`details`、`retryable` 的 typed handler 固定值和 Store 映射以第 5.10 节为唯一事实源；客户端不得只凭 code 猜测是否可重试。
...
### 5.10 `system.ping` / `task.create` / `task.get` typed application handler

本节是这三个方法的库级 application handler 唯一事实源。公共 `handle_*` 函数继续接受 generated typed Envelope，并在 family/discriminator/payload variant 错配时本地 `InputMethodMismatch`；第 5.11 节的 registered dispatcher 是正常调用路径，保证这种错配不可构造。handler 输入必须已经通过对应 Command/Query Envelope Schema、方法 payload Schema 和 `kernel-contracts` typed decode；UTF-8、JSON parse、frame、最大尺寸、clock/backend/ID 不属于 preflight，protocol/auth/method/schema 分类也不得在 typed handler 内重复执行。实现不得在此层接受 raw JSON 或 transport frame，也不得把 typed handler 内部错误伪装成 `invalid_request`。

#### 5.10.1 输入、输出与最终 Schema 门

handler 输入分别是：

- `system.ping`：discriminator 已确认为 `system.ping` 的 `TypedKcpQueryEnvelope` 与 `SystemPingRequest`；
- `task.create`：discriminator 已确认为 `task.create` 的 `TypedKcpCommandEnvelope` 与 `TaskCreateRequest`；
- `task.get`：discriminator 已确认为 `task.get` 的 `TypedKcpQueryEnvelope` 与 `TaskGetRequest`。
...
`TaskCreateOperation` 必须携带完整 typed Envelope 事实、typed request、单一 `accepted_at`，以及 Task、TaskScope、ContentOrigin、receipt、Audit、Event 六个 UUID和 correlation/dedup 两个 opaque ID，足以无损构造现有 `kernel_sqlite::TaskCreateCommand`。精确投影为：Envelope `actor` / `entry_point` / `request_id` / `task_id` / `context` / `expected_revision` / `idempotency_key` 分别进入 `TaskCreateEnvelopeFacts` 的同名语义字段，其中 Envelope `task_id` 进入 `envelope_task_id`；typed `TaskCreateRequest` 原样进入 `request`；generator 的六个 UUID、两个 opaque ID 与第一次 clock 的 `accepted_at` 进入 `TaskCreateAllocation`。backend 不得暴露 `WriteTransaction`、SQLite connection、SQL、canonical hash 或 normalize API；这些仍由 repository 拥有。
...
- `task.create`：第一次也是本次唯一的 `accepted_at` 读取；它同时用于入口检查和全部创建事实时间。未到期后，generator 按 purpose 分配六个 UUID与两个 opaque ID，再调用一次 backend。SQLite `BEGIN IMMEDIATE` 到 commit/rollback 结束之间不得读取 clock、轮询 deadline 或中途取消。backend 返回 Created、Replayed 或错误后都进行第二次 `completed_at` 读取；若 `completed_at >= deadline`，必须优先返回 `deadline_exceeded`，不再暴露该次 backend 结果。Created/Replayed 的既有事实保留，backend 错误则没有本次新提交事实；均不得声称事务被 deadline 取消。

UUID 版本不在 v1 规范中固定：generator 必须返回满足现有 Schema `format: uuid` 的全局唯一 UUID，本次六个 UUID 也必须两两不同；UUID v4 是允许实现但不是协议要求。handler 必须在 backend 调用前验证格式和本次互异性，违反时返回 `internal_error`，不得依赖数据库碰撞间接发现。测试使用 deterministic fake，不得把固定测试 UUID 写成生产派生规则。`correlation_id` 与 `dedup_key` 不是 caller-owned，也不从 `request_id`、`idempotency_key` 或 Task ID 推导：generator 分别生成独立、非空 opaque 值；本次 Audit/Event 共用同一个 correlation，dedup 值对该 Event 全局唯一。repository 继续只校验并写入。
...
名称可按 Rust 社区惯例微调，但三步边界、结果分类和所有权不得合并。`TypedCatalogRequest` 必须携带 generated typed Envelope，不能退回 `Value`、手写 payload 或只保留 method string。`RegisteredRequest` 只包含 `system.ping`、`task.create`、`task.get`，并保证 family、discriminator 与 generated payload variant 已一致。
...
7. 完整 Command/Query Envelope Schema、方法 payload Schema与 generated typed decode。
...
- method 判断必须使用 generated `KCP_COMMAND_METHODS` / `KCP_QUERY_METHODS` 或由同一生成源导出的等价闭集，禁止在 preflight 手写第二份目录；
- `payload` 缺失或非 object，或其**根字段** `schema_version` 缺失、不是 `serde_json::Number` 的 `is_i64() || is_u64()` 整数、`<= 0`，返回 `invalid_request`；这明确拒绝 `1.0` 等浮点表示，以及超出 `i64/u64` 可表示范围而不能保留为该整数形态的数字。根 `schema_version` 是正整数且不等于 `1` 时返回 `unsupported_schema_version`；
- 只有根 `payload.schema_version` 参与版本优先分类。嵌套对象的 `schema_version`、普通业务字段、enum、format、unknown field 或方法 payload 其它约束失败，都由完整 Schema 门归为 `invalid_request`，不得提升为 `unsupported_schema_version`。

前述结构检查完成后，必须用已选 family 的完整 generated Envelope Schema（其条件映射包含方法 payload Schema）校验，再调用 generated `TypedKcpCommandEnvelope::decode` 或 `TypedKcpQueryEnvelope::decode`。普通 caller 输入不满足完整合同，返回 `invalid_request`；成功时首批八方法全部返回 `Accepted(TypedCatalogRequest)`。只有 `Accepted` 后才能调用 `narrow_to_registered`，因此“已知但未实现”不能绕过完整 typed decode。
...
| 根 payload schema integer 不是 `1` | `unsupported_schema_version` | `payload schema version is not supported` |
