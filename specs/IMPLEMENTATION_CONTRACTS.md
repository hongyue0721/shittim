...
- `method_version_bindings`为manifest v2 required typed字段，元素wire shape就是§13.5的完整`MethodVersionBinding`。通用`SchemaRegistry::load`必须允许并完整验证合法非空synthetic registry；production `schemas/manifest.json`在Schema/工具阶段仍显式为空，只由§13.5命名的`validate_production_manifest_stage`断言，直到`V2ProductionWriteCutover`才启用。不得把“production当前为空”实现成通用empty-only loader规则。当前代码仍在`SchemaRegistry::load`内拒绝所有非空binding；这是必须由后续工具提交替换的已知宽松/错误阶段语义，本轮文档不得把它标为完成。
